# Tesla Fleet Management API

[![Built with APIMatic][apimatic-badge]][apimatic-url] [![License: MIT][license-badge]][license-url]

The Tesla Fleet Management API SDK for TypeScript provides typed access to the Tesla Fleet Management API REST APIs from Node.js and the browser.
...
```ts
import sdk = require("tesla-fleet-management-api");

const client = new sdk.TeslaFleetManagementApiClient({
...
This SDK ships a generated **SDK map** — [`sdk-map.md`](sdk-map.md) plus the pages under [`map/operations/`](map/operations/charging.md) — a deterministic, lookup-oriented table of contents of the TypeScript surface, generated alongside the SDK.
...
| Use | For |
| --- | --- |
| **[`sdk-map.md`](sdk-map.md) + [`map/operations/`](map/operations/charging.md)** | Traversing the SDK and working out its surface — locating the operation you need (this SDK exposes **64 operations**), its exact signature, which credential it sends, which channel every request field travels on, which error type it rejects with and how to read it, and the file behind any type. This is the index to consume the SDK from, and the one to reach for first. |
